...
docker rmi -f "${CI_REGISTRY_SERVER_IMAGE}:${IMAGE_VERSION}"
...
docker builder build \
  ...
  -t ${CI_REGISTRY_SERVER_IMAGE}:${IMAGE_VERSION} \
  -f openvpn/Server/openvpn.dockerfile \
  .
...
docker push "${CI_REGISTRY_SERVER_IMAGE}:${IMAGE_VERSION}"
